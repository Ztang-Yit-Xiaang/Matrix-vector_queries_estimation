...
The original is archived at `provenance/source_report.md`; its hash appears in `assembly_manifest.json`.
...
Exact repository paths for each copied report and proof note appear under `provenance_mapping` in `assembly_manifest.json`. The archived Markdown source retains its claim-local links, making the generic local research-record bibliography entry auditable rather than a substitute for source evidence.

## Figures

| File in `figures/` | Role | Authoritative source |
| --- | --- | --- |
| `workflow.png` | Conceptual accounting/workflow | `figures/urop_structure_20260917/` in repository |
| `models.png` | Four risk definitions | Same conceptual package |
| `dependencies.png` | Proof/assumption structure | Same conceptual package |
...
All six are byte-identical copies. The earlier GPT-polished numerical companions are intentionally not used as data figures.

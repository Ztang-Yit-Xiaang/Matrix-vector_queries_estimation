...
Full rank of $S_1$ does not imply a small inverse. Poor conditioning can amplify a nonzero tail and create leakage, which Lemma~\ref{thm:T5} converts to residual energy. The product $S_2S_1^{-1}$, not $S_1^{-1}$ alone, determines the graph error.
...
A deterministic pilot decision based only on those products cannot distinguish the two matrices on this transcript. This is a local identifiability limitation: the alternative can depend on the realized sketch. It does not exclude probabilistic guarantees or additional fresh queries. The positive-oversampling example in Section~\ref{exp:E5} differs from the full-row-rank conditioning mechanism of Lemma~\ref{thm:T5b}.

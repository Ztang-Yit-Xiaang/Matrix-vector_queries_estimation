...
The cache must exist before certification and be charged to construction. A batch of 32 products $Ag_j$ costs 32 certification queries, regardless of action count. Certification observations are not reused as final estimator probes.


\begin{algorithm}
\caption{Common-probe risk diagnostic for preconstructed actions}\label{alg:diagnostic}
\textbf{Input:} cached $Q_x,AQ_x$ for every action $x$, fixed positive $\ell_x$, and integer $s\ge2$.\\
...
If baseline risk is zero, ratios are undefined and no candidate strictly improves it; compare risks directly. Even selecting the smaller numerator cannot refund construction or certification. Useful risk information therefore need not yield a beneficial complete policy.
